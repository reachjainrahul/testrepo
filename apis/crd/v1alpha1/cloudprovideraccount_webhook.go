...
func (r *CloudProviderAccount) ValidateCreate() error {
	...
	case AWSCloudProvider:
		...
		if err := clientK8s.Get(context.TODO(), types.NamespacedName{Namespace: awsConfig.SecretRef.Namespace, Name: awsConfig.SecretRef.Name}, u); err != nil {
	...
	case AzureCloudProvider:
		...
		if err := clientK8s.Get(context.TODO(), types.NamespacedName{Namespace: azureConfig.SecretRef.Namespace, Name: azureConfig.SecretRef.Name}, u); err != nil {
			return fmt.Errorf("unable to get secret: %s", err.Error())
		}
		data := u.Object["data"].(map[string]interface{})
